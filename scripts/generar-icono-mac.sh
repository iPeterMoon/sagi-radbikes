...
# Genera build-assets/icon.icns a partir de un PNG fuente cuadrado (idealmente 1024x1024).
# Uso: ./scripts/generar-icono-mac.sh logo-original.png

set -euo pipefail

ORIGEN="${1:-logo-original.png}"
DESTINO_DIR="build-assets"
...
# png2icns (libicns) solo soporta un set fijo de tamaños de ícono clásico de
# macOS — 64 no es uno de ellos ("Bad dimensions" si se lo pasamos).
